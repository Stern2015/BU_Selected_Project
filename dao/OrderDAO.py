...
'''
Order Management, database operates
'''
class OrderDAO(BaseDAO):
    ...
    def create_order(self, customer_id: str, shipping_address: str, sub_orders_data: List[Dict]) -> Optional[str]:
        ...
        for sub_order_data in sub_orders_data:
            ...
            for item in items:
                ...
                # append stock sql to list
                actual_ops.append((stock_sql, (quantity, product_id)))

        # Execute all operations in one transaction, if tansaction is failed rollback
    ...
    #Get all sub orders belonging to a specific merchant
    def get_vendor_sub_orders(self, merchant_id: str) -> List[Dict]:
        ...
        sql = """
            ...
            FROM sub_orders so JOIN orders o ON so.order_id = o.order_id JOIN UserAccount u ON o.customer_id = u.User_ID
            WHERE so.merchant_id = %s ORDER BY so.created_at DESC
        """
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        # Transaction: cancel order, sub-orders, mark items as removed, and restore the stock
        operations = [
    ...
    def remove_order_item(self, order_item_id: str) -> bool:
        ...
        # stack sql ops into one transaction
        ...
        # exec transaction , if any op fails, rollback all
        return self.tx_manager.execute_transaction(operations)
