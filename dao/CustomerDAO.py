...
# Customer Dao class for Customer related database operations
class CustomerDAO(BaseDAO):
    ...
    def get_product_by_id(self, product_id):
        ...
        # query sql for get product
        sql = """
            SELECT p.*, v.Store_Name, v.Location AS Vendor_Location
            FROM Product p LEFT JOIN Vendor v ON p.Vendor_ID = v.Vendor_ID
    ...
    def get_product_tags(self, product_id):
        # get product tags pairs from table, use inner join for speed
        sql = """
            SELECT t.Tag_ID, t.Name, tg.Position
            FROM Tag t INNER JOIN Tagging tg ON t.Tag_ID = tg.Tag_ID
            WHERE tg.Product_ID = %s ORDER BY tg.Position
        """
        params = (product_id,)
    ...
    def create_order(self, order_id, customer_id, order_date, status, total_payment):
        ...
        return self.executor.execute_update(sql, (order_id, customer_id, order_date, status, total_payment)
    )
...
# Transaction aka suborder
    def create_transaction(self, transaction_id, order_id, vendor_id, payment_amount):
        ...
        return self.executor.execute_update(sql, (transaction_id, order_id, vendor_id, payment_amount))
...
# Order items
    def create_order_item(self, transaction_id, product_id, quantity, price):
        ...
        return self.executor.execute_update(sql, (transaction_id, product_id, quantity, price))
    ...
    # Rating Get rating from this customer for this vendor
    ...
    def get_rating_from_customer(self, customer_id):
        sql = """
            SELECT r.*, v.Store_Name FROM Rating r LEFT JOIN Vendor v ON r.Vendor_ID = v.Vendor_ID
